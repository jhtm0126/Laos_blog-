...
if (libraryProc.passwordCheck(libraryno, pw) == 1) { // 패스워드 일치
  ...
  if (size > 0 && size< 10485760) {
   // 1 ~ 10485760 byte
      file_name = upload.saveFile(fileItem, upDir); // Tomcat이 전송받은 파일을 서버에 저장
...
%>
  ...
  <DIV class='bottom_menu'>
    ...
    %>
      <button type='button' onclick="location.href='./read.jsp?categoryno=<%=categoryno %>&libraryno=<%=libraryno %>&col=<%=col%>&word=<%=word%>'">확인</button>
    <%  
    }
    %>   
