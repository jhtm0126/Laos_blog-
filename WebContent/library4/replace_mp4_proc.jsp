...
if (libraryProc.passwordCheck(libraryno, pw) == 1) { // 패스워드 일치
  ...
  if (sizeMP4 > 0) {
    if (sizeMP4 < 1048576000) { // 1 ~ 1048576000 byte, 1000 MB]
      ...
      // 기존 파일 삭제
      // ------------------------------------------------
      LibraryVO oldLibraryVO = libraryProc.read(libraryno);
      Tool1.deleteFile(upDir, oldLibraryVO.getVideo());
      // ------------------------------------------------
     video = upload.saveFile(fileItemMP4, upDir); // Tomcat이 전송받은 파일을 서버에 저장
     libraryVO.setVideo(video);
 
      count = libraryProc.replace_video(libraryVO); // DBMS 적용
      if (count == 1) {
        %>
