...
<DIV class='content'>
  ...
  <DIV class='aside_menu'>
    <ASIDE style='float: left;'><A href='../category4/list.jsp'>게시판</A>＞<A href='./list.jsp?categoryno=<%=categoryno %>'><%=category_title %></A>＞수정 > 지도 삭제</ASIDE>
  ...
  <DIV class='message'>
    ...
    if (libraryProc.passwordCheck(libraryno, pw) == 1) { // 패스워드 일치
      count = libraryProc.delete_map_info(libraryno);
      if (count == 1) { 
      %>
  ...
  <DIV class='bottom_menu'>
    ...
    %>
      <button type='button' onclick="location.href='./update_form.jsp?categoryno=<%=categoryno %>&libraryno=<%=libraryno %>'">삭제 확인</button>
    <%  
    }
    %> 
