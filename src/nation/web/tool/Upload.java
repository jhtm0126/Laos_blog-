...
public class Upload extends HttpServletRequestWrapper { 
     ...
     * @param threshold 메모리에 저장할 최대크기, -1: 무제한 
     * @param max 업로드할 최대 파일크기, -1: 무제한
            ...
            for (int i = 0 ; i < list.size() ; i++) { 
                ...
                if (fileItem.isFormField()) { // 일반 폼 태그인지 검사 
                    ...
                    if (values == null) { 
                        ...
                        // System.out.println("Upload value 배열에 저장: " + value);
                        // 배열 요소 추가를 위한 배열 생성
    ...
    /** 
     * 서버에 올라온 파일을 서버의 디스크에 저장 
     * 파일명 처리: data.txt -> data_1.txt -> data_2.txt... 
     * @param fileItem 저장할 파일 객체 
     * @param upDir 저장할 서버의 폴더 
     * @return 저장된 파일명 
     */ 
    public String saveFile2(FileItem fileItem, String upDir){ 
